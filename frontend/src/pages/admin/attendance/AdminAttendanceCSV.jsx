...
const AdminAttendanceCSV = () => {
  ...
  const handleUpload2 = async (parsedData) => {
    ...
    for (const stu of validStudents) {
        console.log(stu);
      const response = await fetch("/api/admin/adminattendance", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          totalAttendance: stu.Attendance
        })
      });
    ...
    <div className="flex-[0.8] mt-3">
      <div className="flex text-gray-400 items-center space-x-2">
            ...
            <input
              ...
              onChange={(e) => setValue({ ...value, totalClasses: e.target.value })}
            ...
            />   
            <button
              className="bg-red-500 w-24 h-16 rounded-md text-white hover:scale-105 hover:bg-red-700 transition-all duration-200 "
              type="submit">
              Set Attendance
            </button>        
            </form>
            <div>
            <label htmlFor ="csv" className="text-black">Attendance CSV</label>
...
export default AdminAttendanceCSV